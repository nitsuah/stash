...
| **motor-pool**       | Multi-service AI ops platform             | 🟢 Active — multi-container stack | Sister platform. Shares Ollama. Odysseus monitors its containers, receives outputs from its tools, and can delegate heavy agent tasks to it. Potential: Odysseus kicks off motor-pool tool runs for autonomous coding/content loops.                      | Docker socket (container health, restart); shared Ollama at `8081`; tool output webhooks → Odysseus notes                                                                | 🔴 High                               |
...
| **bb-mcp**            | Blackboard MCP server                     | 🟡 WIP — part of motor-pool      | Deprioritize for now. Will eventually be a good example of a semi-complex MCP layered on top of a local/remote LLM inside motor-pool. Pull from image registry or GitHub when ready.                                                                      | TBD — motor-pool integration first                                                                                                                                      | 🟢 Deprioritized                      |
| **avatar**            | Generative AI model trainer               | 🔵 Deprioritized                  | Interesting as a future motor-pool experience concept (user uploads images → motor-pool runs fine-tune pipeline). Not now.                                                                                                                               | Concept only for now                                                                                                                                                     | 🟢 Low                                |
...
| **opencut-classic**   | Forked video editor                       | 🔵 Exploring                      | Want to play with it first. Potentially becomes an motor-pool experience — its own SLM/container that helps edit AI-generated content from motor-pool tools. One to watch.                                                                               | TBD — evaluate first                                                                                                                                                     | 🟢 Low (watch)                        |
...
|---|---|---|---|
| `motor-pool` | `motor-pool` | `6, 11, 7, 23` | Main app — webhook target for tool outputs; health monitor |
| `agent-dashboard` | `motor-pool-agent-dashboard` | `3000` | UI layer — Odysseus can check if it's up; pull session data |
...
| `tool-content-gen` | `motor-pool-tool-content-gen` | `3200` | Content generation tool — Odysseus can trigger and receive output |
| `tool-website` | `motor-pool-tool-website` | `3201` | Website tool — similar, Odysseus can orchestrate |
...
| 4 | **Docker socket health monitor** | motor-pool containers + Odysseus itself; health checks, auto-restart |
| 5 | **motor-pool tool webhooks** | tool-content-gen and tool-website output → Odysseus notes |
