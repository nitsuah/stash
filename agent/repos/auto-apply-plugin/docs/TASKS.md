...
- [x] Polish the popup into a true job-workspace view.
  ...
  - Progress (2026-09-17, nav/settings/pipeline overhaul): reordered the header nav to Search → Pipeline → Interview Prep → Settings → Profile → Help/Privacy (Analytics kept adjacent to Pipeline), added a hamburger (`#header-menu-toggle`/`#header-secondary-nav`) that collapses everything but the 3 primary buttons below a 700px breakpoint — caught and fixed a real CSS specificity bug in the process (`.header-menu-toggle` vs `.icon-btn` had equal specificity, so the toggle's `display` never actually responded to the media query; fixed by compounding the selector, covered by `tests/e2e/header-nav-responsive.spec.mjs`). Renamed "AI" to "Settings" in all user-facing copy. Settings cards are now `<details>` that collapse into a "✅ Configured" summary once saved (Gemini key, job-search keys, LinkedIn/Google OAuth, custom sources), reopening on click to edit; custom sources stays open-ended (badge-only, no forced collapse) since it's a repeatedly-edited list, not a one-time credential. Search: distinctive empty/no-results state (icon + heading + hint) and the source-chip row is now a real ~3-column grid that wraps instead of a horizontally-scrolling strip. Pipeline: removed the Analytics button from the toolbar filter bar (still reachable from the header nav); Drafts/Submitted/Later-stages columns are now independently collapsible (matching the existing Rejected/Retired pattern) and default to collapsed when empty, without disturbing drag-and-drop (collapsing only hides expanded card previews, never the drop-target elements). Profile: reordered to Core → Preferences → Memory → Sensitive (a flat 2-column grid whose default row-major placement keeps Memory+Sensitive adjacent while Core+Preferences stay on top) and all four now default collapsed; reworked the Save Profile/Parse Resume bar from a fading gradient overlay (which let scrolled content show through, reading as "overlapping") into a proper bordered/backgrounded card. Help & Privacy sections are now collapsed-`<details>` by default; fixed the stale privacy-policy link (was pointing at a root `PRIVACY.md` that had moved to `docs/PRIVACY.md`). Regenerated the screenshot gallery (added Settings/Help/Interview-Prep captures) and reran the full a11y (`axe-core`) suite — no new violations.
...
- [x] Keep local-first autofill and privacy controls trustworthy.
  ...
  - Progress (2026-09-17): fixed a consent-timestamp bug — `readSettingsForm()` (`popup/forms/forms.js`) unconditionally minted `new Date().toISOString()` for `privacy_consent_at` any time the (already-checked) consent box was read, so *any* unrelated "Save Profile" click silently overwrote the user's original consent date. `applyStateToSetupForm()` now stamps the checkbox's `dataset.originalConsent`/`dataset.originalConsentAt` from storage on hydration, and `readSettingsForm()` reuses that recorded timestamp unless this is a genuine first-time accept (checkbox transitioning from never-consented). Loading, updating, and re-saving an already-consented profile no longer touch the date; only first-time signing does. Covered by `tests/consent-timestamp.test.mjs`.
...
- [x] Resolve the Google OAuth client-registration contract for `handleGoogleConnect` (`lib/oauth.js:55-57`, `background/service-worker.js:344-346`).
  - Priority: P2
  ...
  - Progress (2026-09-17): went with option (a) — no code change needed since `launchWebAuthFlow` already produces a `chromiumapp.org` redirect that only a **Web application** client type accepts. Added explicit guidance in the Settings → Google OAuth card (`popup/popup.html`) telling users to register a Web application client (not Desktop) and where to paste the redirect URI; updated the stale "Desktop/Chrome-app type" comment in `lib/oauth.js` to match. This is a product/documentation decision, not a fabricated behavior change — flagging here in case a different tradeoff (option b) is preferred later.
