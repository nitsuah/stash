# eng-mini: darkmoon — 2026-09-09 (REPORT MODE / DRY RUN)

> Report only — no moves executed, no changes made to the target repo. Selected as one of the 2–3 most recently pushed in-scope repos (last commit 2026-09-06T13:15:46-04:00).
...
Unchanged from the 2026-09-01 run — neither move has been executed yet.
...
| `CHANGELOG.md` | — | `FEATURES.md` (self-referential; moves with the file) |
...
Note: `ROADMAP.md` and `TASKS.md` themselves also contain cross-references to each other and to `docs/MULTIPLAYER_SHOOTER_ROADMAP.md` — since those two files are deferred (not moved), no action needed on those references this run.
...
|------|----------------|-------|
| `LICENSE` | Consider `.github/LICENSE` | Per MINI.md — requires human validation of root sensitivity; same open item flagged for kryptos and fire |
...
All three remain unresolved since the 2026-09-01 run.

## Out-of-Scope Observation (not a root-file move, flagging for CLEANUP)

- `test-results/.last-run.json` is still tracked in git and `test-results/` is still not in `.gitignore` — unchanged since the 2026-09-01 run. Recommend the CLEANUP agent evaluate untracking it and adding it to `.gitignore`.
...
1. Execute the two low/medium-risk moves (`CHANGELOG.md`, `FEATURES.md`) on a dedicated `mini/darkmoon/root-hygiene-2026-09-09` branch, updating the 7 references listed above, then open a PR per MINI.md branch/PR governance. These moves have now been proposed twice (2026-09-01 and 2026-09-09) without execution — recommend prioritizing.
2. Bring `LICENSE`, `ROADMAP.md`, `TASKS.md` to the Overseer for an explicit placement decision before any move is attempted (batch across kryptos, darkmoon, and fire).
