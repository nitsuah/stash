...
- [ ] Search for sensitive data patterns: `token`, `password`, `secret`, `.pem`, `.p12`, `credentials` (P0)
...
- [ ] Add architecture baseline documentation — `ARCHITECTURE.md` with module boundaries, execution context, and dependency map (P1)
- [ ] Add script runbook and safety matrix in `README.md` — prerequisites, parameters, dry-run paths, and risk notes per script family (P1)
- [ ] Standardize file naming: rename `ias/Windows-userdata..yml` to remove double-dot anomaly (P2)
- [ ] Add repository-level API contract decision record — `API.md` as concrete spec or explicit "No external API" decision (P3)
...
- [x] PMO audit evidence captured for repository structure and governance files (P1)
- [x] Runtime validation performed on a non-destructive workflow — `cleanup-branches.ps1 -DryRun` executed successfully (P1)
- [x] Re-aligned planning docs to repository reality and original checklist format (P0)
